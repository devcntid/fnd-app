...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 2. Total Corporate: count total donatur yang id_crm = idEmployee dan id_jenis = 3
    const totalCorporateCount = await prisma.corezDonatur.count({
      where: {
        idJenis: 3,
        idCrm: idEmployee,
      },
    ...
    // 3. Total Corporate Aktif: count distinct id_donatur dari corez_transaksi yang id_jenis = 3
    const corporateAktifResult = await prisma.$queryRaw<
      Array<{ count: number }>
    >`
      ...
      WHERE corez_transaksi.id_crm = ${idEmployee}
        AND corez_donatur.id_jenis = 3
    `
    const corporateAktifCount = Number(corporateAktifResult[0]?.count) || 0

    // 4. Total Corporate Tidak Aktif: Total Corporate - Total Corporate Aktif (yang belum bertransaksi)
    const corporateTidakAktifCount = totalCorporateCount - corporateAktifCount

    console.log('Corporate API - Final data:', {
      capaian,
      totalCorporate: totalCorporateCount,
      corporateAktif: corporateAktifCount,
      corporateTidakAktif: corporateTidakAktifCount,
    })
