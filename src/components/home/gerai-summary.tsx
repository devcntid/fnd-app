...
export function GeraiSummary() {
  const [data, setData] = useState<GeraiData | null>(null)
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/gerai?tahun=${selectedYear}&bulan=${selectedMonth}&verified=${selectedVerified}`
        )
  ...
  useEffect(() => {
    ...
    const fetchLeaderboard = async (page = 1, append = false) => {
      try {
        ...
        const response = await fetch(
          `/api/gerai/leaderboard?tahun=${leaderboardYear}&bulan=${leaderboardMonth}&verified=${leaderboardVerified}&page=${page}&limit=5`
        )
        const result = await response.json()
  ...
  const loadMoreLeaderboard = async () => {
    if (!leaderboardLoading && hasMoreLeaderboard) {
      try {
        setLeaderboardLoading(true)
        const response = await fetch(
          `/api/gerai/leaderboard?tahun=${leaderboardYear}&bulan=${leaderboardMonth}&verified=${leaderboardVerified}&page=${
            leaderboardPage + 1
          }&limit=5`
        )
        const result = await response.json()
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
        ...
        <div className="text-center">
          ...
          <p className="text-sm font-semibold text-gray-600">Capaian Gerai</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 text-center border-t border-gray-100 pt-4">
          <div>
            ...
            <p className="text-xs text-gray-600">Total Gerai</p>
          </div>
          <div>
            <p className="text-xl font-bold text-green-600">
              {data?.geraiAktif || 0}
            </p>
            <p className="text-xs text-gray-600">Gerai Aktif</p>
          </div>
          <div>
            <p className="text-xl font-bold text-red-500">
              {data?.geraiTidakAktif || 0}
            </p>
            <p className="text-xs text-gray-600">Gerai Tidak Aktif</p>
          </div>
        </div>
      </div>
