    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/mitra?tahun=${selectedYear}&bulan=${selectedMonth}&verified=${selectedVerified}`
        )
        ...
        console.error('Error fetching Mitra Unggul data:', error)
      } finally {
        if (isMounted) {
          setLoading(false)
        }
  ...
  useEffect(() => {
    ...
    const fetchLeaderboard = async (page = 1, append = false) => {
      try {
        ...
        const response = await fetch(
          `/api/mitra/leaderboard?tahun=${leaderboardYear}&bulan=${leaderboardMonth}&verified=${leaderboardVerified}&page=${page}&limit=5`
        )
        const result = await response.json()
  ...
  const loadMoreLeaderboard = async () => {
    if (!leaderboardLoading && hasMoreLeaderboard) {
      try {
        setLeaderboardLoading(true)
        const response = await fetch(
          `/api/mitra/leaderboard?tahun=${leaderboardYear}&bulan=${leaderboardMonth}&verified=${leaderboardVerified}&page=${
            leaderboardPage + 1
          }&limit=5`
        )
        const result = await response.json()
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4 text-center border-t border-gray-100 pt-4">
          <div>
            ...
            <p className="text-xs text-gray-600">Total Mitra</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-xl font-bold text-red-500">
              {data?.mitraTidakAktif || 0}
            </p>
            <p className="text-xs text-gray-600">Mitra Tidak Aktif</p>
          </div>
        </div>
      </div>

      {/* Leaderboard Mitra Unggul */}
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        <div className="flex justify-between items-center flex-wrap gap-2">
          <h3 className="font-semibold text-lg text-gray-800">
            Leaderboard Mitra Unggul
