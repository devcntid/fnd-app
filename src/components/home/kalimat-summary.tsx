...
export function KalimatSummary() {
  const [data, setData] = useState<KalimatData | null>(null)
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/kalimat?tahun=${selectedYear}&bulan=${selectedMonth}&verified=${selectedVerified}`
        )
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
        ...
        <div className="text-center">
          ...
          <p className="text-sm font-semibold text-gray-600">Capaian Kalimat</p>
        </div>
          ...
          {leaderboardData.length > 0 ? (
            <>
              {leaderboardData.map((item, index) => (
                ...
                >
                  {/* Ranking Badge */}
                  <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-amber-400 to-orange-500 text-white rounded-full flex items-center justify-center text-xs font-bold">
                    {index + 1}
                  </div>
